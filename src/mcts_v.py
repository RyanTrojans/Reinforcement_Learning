...
vf_weights = np.load('weights.npy')
print('vf_weights:', vf_weights.shape)
vf_weights = vf_weights.squeeze(1)

def find_risk_boundary(node):
    print('find_risk_boundary:', node.time)
    def V(state):
        return np.dot(vf_weights, state).item()
...
def MCTS(state, real_time, max_iters=100):
    ...
    for i in range(max_iters):
        ...
        # backup with (15)
        if node.time < H:
            simulation_reward = cur_r + np.dot(vf_weights, node.state_).item()
        else:
            # cur_r = node.reward
...
def Expand(node: TreeNode, action=None):
    ...
    if node.time >= number_steps:
        ...
        newnode.state_ = make_state(env, norm=True)
        # if newnode.time < H:
...
def test():
    ...
    action = 0.04  # L/h action space [0, 0.05]
